...
import com.dbvertex.dilsayproject.Adapter.DislikeAdapter;
...
public class DislikeFragment extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState)
    {
        ...
        disLikeDTOArrayList = new ArrayList<>();
        dislikeAdapter = new DislikeAdapter(getActivity(), disLikeDTOArrayList);

        RecyclerView.LayoutManager mLayoutManager = new GridLayoutManager(getActivity(), 2);
        recycler_view.setLayoutManager(mLayoutManager);
        recycler_view.addItemDecoration(new GridSpacingItemDecoration(2, dpToPx(5), true));
        recycler_view.setItemAnimator(new DefaultItemAnimator());
        recycler_view.setAdapter(dislikeAdapter);

        loadDisLikeList();

    }
    private void loadDisLikeList()
    {
                        ...
                        try
                        {
                            ...
                            if (status == 200 && message.equals("success"))
                            {
                                ...
                                for (int x = 0; x < data_list.length(); x++)
                                {
                                    ...
                                    disLikeDTO.setAge(dob);
                                    disLikeDTO.setCaste(religion);
                                    disLikeDTO.setId(id);
                                    disLikeDTO.setImage(fb_image);
                                    disLikeDTO.setLocation(raised_in);
                                    disLikeDTO.setName(name);
                                    disLikeDTO.setProfession(career);
...
//                                        }
//                                    }
                                    disLikeDTOArrayList.add(disLikeDTO);
                                }
                                recycler_view.setAdapter(dislikeAdapter);
                            }
